...
sockunion_socket (union sockunion *su)
{
  ...
  if (sock < 0)
    {
      mlog (NULL, LOG_WARNING, "Can't make socket : %s", safe_strerror (errno));
      return -1;
    }
...
sockunion_stream_socket (union sockunion *su)
{
  ...
  if (sock < 0)
    mlog (NULL, LOG_WARNING, "can't make socket sockunion_stream_socket");
		...
		union sockunion *su_addr)
{
  ...
  if (ret < 0)
    mlog (NULL, LOG_WARNING, "can't bind socket : %s", safe_strerror (errno));
...
sockopt_reuseaddr (int sock)
{
  ...
  if (ret < 0)
    {
      mlog (NULL, LOG_WARNING, "can't set sockopt SO_REUSEADDR to socket %d", sock);
      return -1;
    }
  return 0;
}
...
sockopt_reuseport (int sock)
{
  ...
  if (ret < 0)
    {
      mlog (NULL, LOG_WARNING, "can't set sockopt SO_REUSEPORT to socket %d", sock);
      return -1;
    }
  return 0;
}
  ...
  if (family == AF_INET)
    {
      ...
      if (ret < 0)
	{
	  mlog (NULL, LOG_WARNING, "can't set sockopt IP_TTL %d to socket %d", ttl, sock);
	  return -1;
	}
      return 0;
    }
...
sockunion_getsockname (int fd)
{
  ...
  if (ret < 0)
    {
      mlog_warn ("Can't get local address and port by getsockname: %s",
		 safe_strerror (errno));
      return NULL;
    }
